...
exports.country = function (country_code) {
	...
	return internal.downloadUnzip(country_code).then(function (file) { return path.join(file, country_code + '.txt') });
};
...
internal.downloadUnzip = function (name) {
	var file = path.join(TEMP_DIR, name);
	return internal.downloadFile(name + '.zip')
		.delay(1000 * 2)
		.then(function () {
			return internal.unzip(path.join(TEMP_DIR, name + '.zip'), file).then(function () { return file });
		});
};
